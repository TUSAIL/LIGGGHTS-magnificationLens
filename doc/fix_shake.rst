...
fix\_shake.cpp).  The *a* keyword lists angle types.  If both bonds in
...
.. warning::
   ...
   This command works by using the current forces on
   atoms to calculate an additional constraint force which when added will
   ...
   define fixes (e.g. :doc:`fix efield <fix_efield>`) that add additional
...
:doc:`Section\_accelerate <Section_accelerate>` of the manual.  The
