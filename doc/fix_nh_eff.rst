...
.. warning::
   ...
   for eFF when defining the pair\_style (see :doc:`pair\_style eff/cut <pair_eff>` to understand these settings), one
...
.. warning::
   ...
   command, so the user must allow for these degrees of freedom to
...
.. warning::
   ...
   The temperature for systems (regions or groups) with
