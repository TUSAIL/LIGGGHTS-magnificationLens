...
:doc:`Section\_accelerate <Section_accelerate>`.
...
:doc:`kspace\_modify <kspace_modify>` command, rather than let LAMMPS set
...
.. warning::
   ...
   single-precision FFTs by using the compiler switch -DFFT\_SINGLE for
   the FFT\_INC setting in your lo-level Makefile.  This setting also
   ...
   precision.  This option can speed-up long-range calculations,
   particularly in parallel or on GPUs.  The use of the -DFFT\_SINGLE flag
   is discussed in :ref:`this section <start_2_4>` of the
   manual. MSM does not currently support the -DFFT\_SINGLE compiler switch.
...
grid size, and then use the :doc:`kspace\_modify <kspace_modify>` *mesh* 
...
with those packages.  See the :ref:`Making LAMMPS <start_3>`
...
The only exception is if the slab option is set with :doc:`kspace\_modify <kspace_modify>`, 
