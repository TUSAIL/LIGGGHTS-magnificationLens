...
* LJ\_flag = 0/1 to turn off/on the LJ term (AIREBO only, optional)
* TORSION\_flag = 0/1 to turn off/on the torsion term (AIREBO only, optional)
...
discussed below, 2nd generation REBO is closely related to the initial
...
the two optional flag arguments to the pair\_style command are
...
Only a single pair\_coeff command is used with the *airebo* or *rebo*
