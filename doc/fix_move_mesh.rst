...
* style = *linear* or *linear/variable* or *wiggle* or *riggle* or *rotate* or *rotate/superposable* or *rotate/variable* or *viblin* or *vibrot*
  ...
  .. parsed-literal::
       ...
       *riggle* args = origin Px Py Pz axis ax ay az period per amplitude ampl
         ...
         amplitude = obligatory keyword
         ampl = amplitude of riggle movement (degree)
...
but uses three variables so that the velocity can be time-dependent.
...
axis *R* = (Rx,Ry,Rz) that goes through a point *P* = (Px,Py,Pz). 
The *period* of the oscillation is specified as well as the *amplitude*
in degrees (&deg;). This style also sets the velocity of
...
(ax,ay,az) that goes through a point *origin* = (Px,Py,Pz). The *period* of the 
