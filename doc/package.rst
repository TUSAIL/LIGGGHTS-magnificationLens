...
below, if you use the "-sf gpu" or "-sf omp" :ref:`command-line options <start_7>` to enable use of these styles,
...
cannot be used with :doc:`hybrid <pair_hybrid>` pair styles.  GPU
...
GPU. In this case, :doc:`hybrid <pair_hybrid>`, :doc:`bond <bond_style>`,
...
The *threads\_per\_atom* keyword allows control of the number of GPU
...
:doc:`coul/wolf <pair_coul>` or :doc:`coul/dsf <pair_coul>`. For very large cutoffs,
...
The *test* keyword will output info for the specified atom at
...
when *OMP\_NUM\_THREADS* is not set, which can lead to extremely bad
performance.
...
If the "-sf gpu" :ref:`command-line switch <start_7>` is
...
2".  This is the case whether the "-sf cuda" :ref:`command-line switch <start_7>` is used or not.

If the "-sf omp" :ref:`command-line switch <start_7>` is
