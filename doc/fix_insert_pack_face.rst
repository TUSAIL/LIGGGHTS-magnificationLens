...
* pack\_face\_keywords = *region* or *massflow\_face* or *cg* or *type\_offset* or *ntry\_mc*
  ...
  .. parsed-literal::
       ...
       *massflow_face* values = fix-ID
         fix-ID =  ID of a :doc:`fix massflow/mesh/face <fix_massflow_mesh_face>` or :doc:`fix massflow/mesh/face/universe <fix_massflow_mesh_face>` command
       *cg* value = cg
         cg = coarse grain ratio of particles to insert (N/A to universe version)
       *type_offset* value = to
         to = offset in atom type compared to particles measured by :doc:`fix massflow/mesh/face <fix_massflow_mesh_face>`
       *ntry_mc* values = n
       ...
       *chemistry* value = *yes* or *no*
         yes = to set the reduction state of the inserted particles

* one ukeyword/uvalue pair must be appended for the universe version of this command
* ukeywords = *receive\_from\_partition*
...
Insert particles recorded by :doc:`fix massflow/mesh/face <fix_massflow_mesh_face>`
...
which can be altered via the *type\_offset* keyword. The particles are assigned to
...
The *massflow\_face* must be used to specify a
:doc:`fix massflow/mesh/face <fix_massflow_mesh_face>` command that
...
The *type\_offset* option can be used to change the atom type of the
...
option.
Furthermore, *massflow\_face* needs to specify a fix of style *massflow/mesh/face/universe*
...
The defaults are *maxattempt* = 50, *all\_in* = *no*\ , *start* = next time-step,
*cg* = 1, *type\_offset* = 0, *ntry\_mc* = 100000
