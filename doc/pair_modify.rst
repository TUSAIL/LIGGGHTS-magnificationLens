...
:doc:`thermo\_style <thermo_style>` command).  Their effect will also be
included in constant NPT or NPH simulations where the pressure
influences the simulation box dimensions (e.g. the :doc:`fix npt <fix_nh>` and :doc:`fix nph <fix_nh>` commands).  The formulas
...
:doc:`pair\_style <pair_style>`, but a Kspace-compatible pair\_style is
required if you also want to define a
:doc:`kspace\_style <kspace_style>`.  This keyword gives you that option.
