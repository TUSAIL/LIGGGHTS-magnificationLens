...
* keyword = *pos* or *vel* or *id* or *force* or *force\_normal* or *force\_tangential* or *torque* or *history* or *contactArea* or *delta*
...
.. warning::
   ...
   For accessing particle-wall contact data, only mesh
   walls (see :doc:`fix mesh/surface <fix_mesh_surface>`) can be used.
   For computing particle-wall (compute wall/gran/local), the code will
   automatically look for a :doc:`fix wall/gran <fix_wall_gran>` command
   ...
   dedicated fix mesh command. For how to output the triangle id,
   see "dump mesh/gran/VTK command"dump.html.

The output *pos* is the particle positions (6 values) in distance
:doc:`units <units>`. Keyword *vel* will do the same for velocities.
...
*force*\ , *force\_normal*, *force\_tangential* and *torque* are the total
...
:doc:`units <units>`. Note that the normal and tangential components
are not necessarily exactly equal to the forces added by
:doc:`the normal and tangential model used <Section_gran_models>`, but
...
in distance\^2 :doc:`units <units>`. Note that *contactArea* is based on
...
:doc:`units <units>` .The output *heatFlux* (available only
if a :doc:`fix heat/gran <fix_heat_gran_conduction>` is used to compute  heat
fluxes) will output the per-contact conductive heat flux area, in
energy/time :doc:`units <units>`.

.. warning::

   The data associated to the different keywords is output in the
   following order: *pos*\ , *vel*\ , *id*\ , *force*\ , *force\_normal*,
   *force\_tangential*, *torque*\ , *history*\ , *contactArea*\ , *delta*\ , *heatFlux*\ .
   This is independent of the order in which the keywords are
   specified.
...
.. warning::
   ...
   This compute will, when invoked, issue a call to
   the pair or wall contact models to calculate what would be the contact
   forces given the current positions, velocities etc.
   Since this compute is typically executed when output is created (at the end of
