...
* limitstyle = absolute or radius\_ratio
...
If limitstyle *absolute* is used, *xmax* is applied directly.
If limitstyle *radius\_ratio* is used, a maximum distance per step 
of *xmax*\ \*radius is applied for each atom. This can be useful for
the simulation of poly-disperse systems. Note that this option requires
the atom radius to be stored by using an appropriate atom style.
...
.. warning::
   ...
   You should not use :doc:`fix shake <fix_shake>` in
   conjunction with this fix.  That is because fix shake applies
   constraint forces based on the predicted positions of atoms after the
