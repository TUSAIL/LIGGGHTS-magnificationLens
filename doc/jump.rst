...
by using the :ref:`-in command-line argument <start_7>` or
the :ref:`-var command-line argument <start_7>` to pass
the script name as a variable to the input script In the latter case,
the "fname" :doc:`variable <variable>` could be used in place of SELF.
E.g.
...
then continues iterating through the outer loop.
