...
.. warning::
   ...
   For each TIP4P water molecule in your system, the atom
...
:doc:`kspace\_style ewald/n <kspace_style>` command.  The specified LJ
...
in reciprocal space via the Kspace style.  If *flag\_lj* is set to
*cut*\ , the LJ interactions are simply cutoff, as with :doc:`pair\_style lj/cut <pair_lj>`.  If *flag\_lj* is set to
...
style, including *ewald/n* of the :doc:`kspace\_style <kspace_style>`
command.  Note that if *flag\_lj* is also set to long, then only the
*ewald/n* KSpace style can perform the long-range calculations for
both the LJ and Coulombic interactions.  If *flag\_coul* is set to
