...
| 13.1 :ref:`Common problems <err_1>`
| 13.2 :ref:`Reporting bugs <err_2>`
| 13.3 :ref:`Error & warning messages <err_3>`
|
...
Check the `New features and bug fixes <https://www.lammps.org/bug.html>`_
section of the `LAMMPS WWW site <lws_>`_ to see if the bug has already been reported or fixed or the
`Unfixed bug <https://www.lammps.org/unbug.html>`_ to see if a fix is
pending.

Check the `mailing list <https://www.lammps.org/mail.html>`_
...
*Cannot use fix gcmc with a triclinic box*
   Fix gcmc is set up to run with orthogonal boxes only. Simulations with
...
*Dispersion PPPMDisp order has been reduced below minorder*
   ...
   command to prevent changing of the dispersion order.
...
*Atom with molecule ID = 0 included in compute molecule group*
   The group used in a compute command that operates on molecules
...
*One or more compute molecules has atoms not in group*
   The group used in a compute command that operates on molecules does
