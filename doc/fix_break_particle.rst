...
* dist-ID = ID of a :doc:`fix\_particledistribution\_discrete <fix_particledistribution_discrete>` to be used for fragment insertion
* one or more general keyword/value pairs can be appended
* general\_keywords = *verbose* or *every* or *start*
  ...
  .. parsed-literal::
       ...
       *start* value = ts
         ts = time-step at which fragmentation should start (positive integer larger than current time-step)
...
* break\_keywords = *breakability* or *fMat* or *energy\_threshold* or *force\_threshold* or *von\_mises\_stress* or *min\_radius*
* break\_keywords = where exactly one out of *energy\_threshold* or *force\_threshold* or *von\_mises\_stress* has to be defined
  ...
  .. parsed-literal::
       ...
       *min_radius* value = mrad
         mrad = minimum radius of particles below which no breakage will occur (positive float)
...
brittle granular particles (see :ref:`Bruchmüller <Bruchmueller2011>` and :ref:`Esnault <Esnault2013>`).
...
The :doc:`fix\_particledistribution\_discrete <fix_particledistribution_discrete>`
in turn must refer to exactly one :doc:`fix\_particletemplate\_fragments <fix_particletemplate_fragments>`
...
can be based either on impact energy (keyword *energy\_threshold*),
a maximum amount of force (keyword *force\_threshold*), or the von Mises stress
(keyword *von\_mises\_stress*). Exactly one out of the keywords *energy\_threshold*,
*force\_threshold*, *von\_mises\_stress* must be defined.
...
and :doc:`fix\_particletemplate\_fragments <fix_particletemplate_fragments>`
...
Use the *energy\_threshold* keyword to apply the impact energy criterion.
...
.. image:: Eqs/fix_break_particle_p_energy.png
   :align: center

Use the *force\_threshold* keyword to apply the maximum force criterion.
...
.. image:: Eqs/fix_break_particle_p_force.png
   :align: center

Use the *von\_mises\_stress* keyword to apply the von Mises stress criterion.
...
.. image:: Eqs/fix_break_particle_p_stress.png
   :align: center
...
Any of *fMat*\ , *energy\_threshold*, *force\_threshold* and *von\_mises\_stress*
can be specified as an equal-style or atom-style :doc:`variable <variable>`.
If the value is a variable, it should be specified as v\_name, where name is
...
the *min\_radius* option can be used to define a minimum particle radius,
...
This fix requires the :doc:`pair\_style gran <pair_gran>` to be used with either
...
:doc:`fix\_particledistribution\_discrete <fix_particledistribution_discrete>`,
:doc:`fix\_particletemplate\_fragments <fix_particletemplate_fragments>`

Default
"""""""

The defaults are verbose = yes, start = next time-step, breakability = uniform, min\_radius = 0
