...
perform "dynamic" balancing, see the :doc:`fix balance <fix_balance>`
...
Note that the :doc:`processors <processors>` command gives you control
...
.. warning::
   ...
   solver used via the :doc:`kspace\_style <kspace_style>` command.  Thus
...
"create\_box" or "read\_data" or "read\_restart" command and is
...
to how the :doc:`fix balance <fix_balance>` command operates.  Note that
...
.. warning::
   ...
   At each rebalance operation, the RCB for each cutting
...
.. warning::
   ...
   If a portion of your system is a perfect lattice,
   e.g. the initial system is generated by the
   :doc:`create\_atoms <create_atoms>` command, then the balancer may be
...
:doc:`fix mesh/surface <fix_mesh_surface>` command)
