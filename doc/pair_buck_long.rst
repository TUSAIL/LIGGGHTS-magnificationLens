...
the :doc:`kspace\_style ewald/n <kspace_style>` command.  The specified
...
*flag\_buck* is set to *cut*\ , the Buckingham interactions are simply
cutoff, as with :doc:`pair\_style buck <pair_buck>`.
...
command.  Note that if *flag\_buck* is also set to long, then only the
*ewald/n* Kspace style can perform the long-range calculations for
both the Buckingham and Coulombic interactions.  If *flag\_coul* is set
...
This pair style supports the :doc:`pair\_modify <pair_modify>` table and
