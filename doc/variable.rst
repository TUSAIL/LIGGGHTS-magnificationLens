...
.. warning::
   ...
   Every time a set of per-atom lines is read, the value
...
Specifically, a formula can contain numbers, thermo keywords, math
...
+--------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Math functions     | sqrt(x), cbrt(x), exp(x), ln(x), log(x), abs(x),      sin(x), cos(x), tan(x), asin(x), acos(x), atan(x), atan2(y,x),      random(x,y,z), normal(x,y,z), ceil(x), floor(x), round(x), ramp(x,y),      stagger(x,y), logfreq(x,y,z), stride(x,y,z), vdisplace(x,y),      swiggle(x,y,z), cwiggle(x,y,z) |
+--------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Group functions    | count(ID), mass(ID), charge(ID), xcm(ID,dim),      vcm(ID,dim), fcm(ID,dim), bound(ID,dir), boundid(ID,dir),     idbound(ID,dir), gyration(ID), ke(ID), angmom(ID,dim), torque(ID,dim),      inertia(ID,dimdim), omega(ID,dim)                                                                        |
+--------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Region functions   | count(ID,IDR), mass(ID,IDR), charge(ID,IDR),      xcm(ID,dim,IDR), vcm(ID,dim,IDR), fcm(ID,dim,IDR),      bound(ID,dir,IDR), boundid(ID,dir,IDR), idbound(ID,dir,IDR), gyration(ID,IDR), ke(ID,IDR),      angmom(ID,dim,IDR), torque(ID,dim,IDR),      inertia(ID,dimdim,IDR), omega(ID,dim,IDR)      |
+--------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Special functions  | sum(x), min(x), max(x), ave(x), trap(x), gmask(x), rmask(x),      grmask(x,y), next(x)                                                                                                                                                                                                                |
+--------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Atom values        | id[i], mass[i], type[i], x[i], y[i], z[i],              vx[i], vy[i], vz[i], fx[i], fy[i], fz[i]                                                                                                                                                                                                      |
+--------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
if logfreq(100,4,10) is used in a variable by the :doc:`dump\_modify every <dump_modify>` command, it will generate this sequence of
output timesteps:
...
argument which is of the form "c\_ID" or "c\_ID[N]" or "f\_ID" or
"f\_ID[N]".  The first two are computes and the second two are fixes;
...
time-series data, like that generated by the :doc:`fix ave/correlate <fix_ave_correlate>` command.
...
as inputs to a variable by using the :doc:`compute property/atom <compute_property_atom>` command and then specifying
...
+------------+-----------------------------------+
| v\_name    | scalar, or per-atom vector        |
+------------+-----------------------------------+
| v\_name[I] | atom I's value in per-atom vector |
+------------+-----------------------------------+
...
quantity from a :doc:`compute <compute>` that is not current, LAMMPS
...
information create global maps.  The :doc:`atom\_modify map <atom_modify>` command can override the default.
