...
.. warning::
   ...
   To use *rigid/small* the ghost atom cutoff must be
   ...
   this distance, then you should use the :doc:`communicate cutoff <communicate>` command with a setting epsilon larger than
   the distance.
...
.. warning::
   ...
   other time-integration fixes (e.g. :doc:`fix nve <fix_nve>`, :doc:`fix nvt <fix_nh>`, :doc:`fix npt <fix_nh>`), or you will be integrating
...
:ref:`(Hoover) <Hoover>` and :ref:`(Martyna) <Martyna>`, which thermostats both
...
.. note::
   ...
   Currently the *rigid/npt* and *rigid/nph* styles do not support
...
.. warning::
   ...
   When the *langevin* keyword is used with fix rigid
...
.. warning::
   ...
   There are alternate ways to thermostat a system of
   rigid bodies.  You can use :doc:`fix langevin <fix_langevin>` to treat
...
* Use :doc:`fix press/berendsen <fix_press_berendsen>` to compute the
  ...
  rigid bodies.  Use :doc:`fix nvt <fix_nh>` (or any other thermostat) for
...
fixes are written to :doc:`binary restart files <restart>`.  For style
