...
pair\_style line/lj/omp command
===============================
...
represent the segment is calculated as N = L/sigma\_II, rounded up to
an integer value.  Thus if L is not evenly divisibly by sigma\_II, N is
incremented to include one extra sphere.  In this case, the spheres
must be slightly smaller than sigma\_II so as not to overlap, so a new
...
computed using only sigma\_II values, specific to the line segment's
type, this means that any specified sigma\_IJ values (for I != J) are
effectively ignored.
...
LAMMPS was built with that package.  See the :ref:`Making LAMMPS with optional packages <start_3>` section for more info.
