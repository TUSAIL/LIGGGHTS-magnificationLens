...
* thresh = stop balancing when this imbalance threshold is reached
...
Note that the :doc:`processors <processors>` command gives you some
...
the :doc:`neighbor <neighbor>` and :doc:`neigh\_modify <neigh_modify>`
command settings.
...
use the :doc:`balance <balance>` command before starting a run, so that
...
.. warning::
   ...
   At each rebalance operation, the RCB operation for
...
:doc:`fix mesh/surface <fix_mesh_surface>` command), the following 
