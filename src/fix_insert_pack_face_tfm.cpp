...
#include <stdlib.h>
#include <string.h>
#include "fix_insert_pack_face_tfm.h"
#include "atom.h"
#include "atom_vec.h"
#include "force.h"
#include "update.h"
...
FixInsertPackFaceTFM::FixInsertPackFaceTFM(LAMMPS *lmp, int narg, char **arg) :
  FixInsert(lmp, narg, arg)
{
  ...
  while (iarg < narg && hasargs) {
    ...
    if (strcmp(arg[iarg],"region") == 0) {
      ...
      if (iarg+2 > narg) error->fix_error(FLERR,this,"");
      int ifix = modify->find_fix(arg[iarg+1]); //face data fix id name
      if (strcmp(style,"insert/pack/face/tfm") == 0) {
        if (ifix < 0 || strcmp(modify->fix[ifix]->style,"massflow/mesh/face/tfm"))
          error->fix_error(FLERR,this,"Fix insert/pack/face/tfm requires you to define a valid ID for a fix of type massflow/mesh/face/tfm");
        else
          massflowface = static_cast<FixMassflowMeshFaceTFM*>(modify->fix[ifix]); 
      }
      int n = strlen(arg[iarg+1]) + 1;
      ...
      iarg += 2;
      hasargs = true;
    } else if(strcmp(arg[iarg],"start_index") == 0) {
        if(narg < iarg+2)
           error->fix_error(FLERR,this,"Illegal keyword entry for 'start_index'");
     iarg++;
     startIndex_ = atoi(arg[iarg]);
     if(startIndex_ < 0)
       error->fix_error(FLERR,this,"'start_index' value must be > 0");
     iarg++;
     hasargs = true;
    } else if(strcmp(arg[iarg],"end_index") == 0) {
        if(narg < iarg+2)
           error->fix_error(FLERR,this,"Illegal keyword entry for 'end_index'");
     iarg++;
     endIndex_ = atoi(arg[iarg]);
     if(endIndex_ < 0)
       error->fix_error(FLERR,this,"'end_index' value must be > 0");
     iarg++;
     hasargs = true;
     ...
     hasargs = true;
      ...
      iarg += 2;
      hasargs = true;
    } else if (strcmp(arg[iarg],"dilueTodenseTransition") == 0) {
        if (iarg+2 > narg) error->fix_error(FLERR,this,"not enough arguments for dilueTodenseTransition solids volume fraction");
        alphaMinSolid_ = atof(arg[iarg+1]);
        iarg += 2;
        hasargs = true;
      } else if (strcmp(style,"insert/pack/face/tfm") == 0) {
...
FixInsertPackFaceTFM::~FixInsertPackFaceTFM()
{
...
void FixInsertPackFaceTFM::post_create_per_face_data_TFM()
{
  int nfaces = endIndex_ - startIndex_ +1;
  if (nfaces > 0) {
    fraction_face_local.resize(nfaces, 1.0);
    min_face_extent_local.resize(nfaces, 1.0);
...
void FixInsertPackFaceTFM::post_create()
{
  post_create_per_face_data_TFM();
...
void FixInsertPackFaceTFM::init_defaults()
{
  massflowface = NULL;
  idmassflowface = NULL;
  ins_region = NULL;
  ins_region_mesh_hex = NULL;
  fraction_face_local_all = NULL;
  min_face_extent_local_all = NULL;
...
void FixInsertPackFaceTFM::init()
{
  FixInsert::init();
...
void FixInsertPackFaceTFM::calc_insertion_properties()
{
  ...
  calc_region_volume_local();//portion of each cell in each processor
...
void FixInsertPackFaceTFM::calc_region_volume_local()
{
...
------------------------------------------------------------------------- */
void FixInsertPackFaceTFM::receive_ninsert_this(int& ninsert_this)
{
  ...
  if (massflowface) {
    ninsert_this = static_cast<int>(massflowface->compute_vector(1));
  }
}

int FixInsertPackFaceTFM::calc_ninsert_this()
{
  int ninsert_this = 0;
...
double FixInsertPackFaceTFM::insertion_fraction() //develop the same fraction of the cells in the procs for cells
{
    ...
    double volRatio_ = 0.0;
    if (checkFlag == -1)
    {
        if (comm->me == 0) printf ("No insertion at the first coupling interval! \n");
     } else {
  ...
  for (std::map<int,int>::const_iterator it=faceids.begin(); it!=faceids.end(); ++it) {
       if ((it->second >= startIndex_) && (it->second <= endIndex_)){
             fraction_face_local[it->second - startIndex_] = insertion_fraction_face(it->first);
       }
  }

  // mc method may be too vague -> normalize fraction
  int nfaces = endIndex_ - startIndex_ + 1;//faceids.size();
  ...
  for (int i=0; i<nfaces; ++i) {
    ...
    volRatio_ = region_volume_local/region_volume;
  }
    return volRatio_;
}
...
int FixInsertPackFaceTFM::get_face_index_check(int face_id) //cell array index
{
  // check face_id
  if (massflowface->get_face_ids().find(face_id) == massflowface->get_face_ids().end())
    error->fix_error(FLERR,this,"invalid face id");

  return massflowface->get_face_ids().at(face_id) - startIndex_;
}

double FixInsertPackFaceTFM::insertion_fraction_face(int face_id)
{
...
int FixInsertPackFaceTFM::distribute_ninsert_this(int ninsert_this)
{
 ...
 int nfaces = fraction_face_local.size();
  if (counter_ < 2) {
      ...
      printf("Counter_ has been Called! \n");
  }
  ...
  if (comm->me == 0) printf("Total number of particles to be inserted is: %d \n", ninsert_this);
  if(ninsert_this > 0) {
    // loop over faces
    for (int iface=startIndex_; iface<=(startIndex_ + nfaces -1); ++iface) {
      ...
      for (int idist=0; it_dist!=distributions_face[iface].end(); ++it_dist, ++idist) {
        ...
        for (int iproc = 0; iproc < nprocs; ++iproc) {
          double procvolfracface = fraction_face_local_all[iproc*nfaces+iface - startIndex_];
          double procminfaceextent = min_face_extent_local_all[iproc*nfaces+iface - startIndex_];
         ...
         while(nparts > 0 && nparts + accumulatedTemplateParticles_facei[idist] > nparticles) {
          ...
          double volume = nparts*volume_single;
           //B.E: commented to be able to insert small micrometer particles, this comparison condition was causing a problem in insertion!
...
*/
          ...
          distributions_face_local_all[iproc][iface - startIndex_].push_back(nparts);
          if (iproc == me)
        ...
        while (nparticles - static_cast<double>(accumulatedTemplateParticles_facei[idist]) > 0.5) {
          ...
          for (int iproc = 0; iproc < nprocs; ++iproc) {
            ...
            if (diameter < procminfaceextent &&
                ...
                double assigned = static_cast<double>(distributions_face_local_all[iproc][iface - startIndex_].back());
          ...
          distributions_face_local_all[correcting_proc][iface - startIndex_].back()++;
  ...
  // 1.0 corresponds to cg
  fix_pddf->set_distribution_local(distributions_face, distributions_face_local_all[me], 1.0, type_offset, startIndex_, endIndex_);
...
inline int FixInsertPackFaceTFM::is_nearby(int i)
{
...
BoundingBox FixInsertPackFaceTFM::getBoundingBox() const
{
...
int FixInsertPackFaceTFM::calc_maxtry(int ninsert_this_local)
{
...
void FixInsertPackFaceTFM::x_v_omega(int ninsert_this_local,int &ninserted_this_local, int &ninserted_spheres_this_local, double &mass_inserted_this_local)
{
  ...
  // always do overlap check
  {
    ...
    massflowface = static_cast<FixMassflowMeshFaceTFM*>(modify->fix[ifix]);
    if (!massflowface)
      error->fix_error(FLERR, this, "Failed to locate fix massflow/mesh/face/tfm");
    ...
    if ((it->second >= startIndex_) && (it->second <= endIndex_)){
      ...
      int particle_this_face_local = fix_pddf->pti_list_face_local[iHex].size();
      int ninserted_this_face_local = 0;
      particle_this_face = massflowface->compute_array_by_id(it->first, 1);
      v_insert[0] = massflowface->compute_array_by_id(it->first, 3) + random->gaussian() * sqrt(massflowface->compute_array_by_id(it->first, 2));
      v_insert[1] = massflowface->compute_array_by_id(it->first, 4) + random->gaussian() * sqrt(massflowface->compute_array_by_id(it->first, 2));
      v_insert[2] = massflowface->compute_array_by_id(it->first, 5) + random->gaussian() * sqrt(massflowface->compute_array_by_id(it->first, 2));
      ...
      while (ninserted_this_face_local < particle_this_face_local) {
        pti = fix_pddf->pti_list_face_local[iHex][ninserted_this_face_local];
        const double rbound = 0.01 * pti->r_bound_ins;
        ntry = 0;
        int nins = 0;
        ...
        while (nins == 0) {
          ...
          if(check_ol_flag) {
            ...
            nins = pti->set_x_v_omega(pos,v_toInsert,omega_insert,quat_insert);
            break;
          }
        }
        if (nins > 0) {
          ...
          fix_pddf->pti_list_face_local[iHex].erase(fix_pddf->pti_list_face_local[iHex].begin()+ninserted_this_face_local);//fix_pddf->pti_list_face_local[iface].erase(fix_pddf->pti_list_face_local[iface].begin()+ninserted_this_face_local);
...
void FixInsertPackFaceTFM::restart(char *buf)
{
