                            ...
                            double *result, int stride, int sumflag)
{
  ...
  if (style[ivar] == ATOM) {
    ...
    collapse_tree(tree); //NP modified R.B.
                            ...
                            double *argstack, int &nargstack)
{
  ...
  if (tree) {
    ...
    Tree *argtree;
    if (narg == 1) {
      evaluate(arg1,&argtree); //NP modified R.B.
      ...
      newtree->left = argtree;
      newtree->middle = NULL;
      evaluate(arg2,&argtree); //NP modified R.B.
      ...
      evaluate(arg3,&argtree); //NP modified R.B.
      newtree->right = argtree;
    }
    treestack[ntreestack++] = newtree;
  } else {
                             ...
                             double *argstack, int &nargstack)
{
  ...
  } else if (strcmp(word,"boundid") == 0) {
    int minmax[6];
    if (narg == 2) group->boundids(igroup,minmax);
    else if (narg == 3) group->boundids(igroup,minmax,region_function(arg3));
    else error->all(FLERR,"Invalid group function in variable formula");
    if (strcmp(arg2,"xmin") == 0) value = minmax[0];
    else if (strcmp(arg2,"xmax") == 0) value = minmax[1];
    else if (strcmp(arg2,"ymin") == 0) value = minmax[2];
                              ...
                              double *argstack, int &nargstack)
{
  ...
  if (index >= 0 && index < atom->nlocal) {
    ...
    if (flag == 0) {
      ...
      else if ((strcmp(word,"tqz") == 0) && atom->torque_flag) mine = atom->torque[index][2]; //NP modified C.K. end
...
int Variable::is_atom_vector(char *word)
{
  ...
  if ((strcmp(word,"tqz") == 0) && atom->torque_flag) return 1; //NP modified C.K. end
  return 0;
}
                           ...
                           Tree **treestack, int &ntreestack)
{
  ...
  else if ((strcmp(word,"omegax") == 0) && atom->omega_flag) newtree->array = &atom->omega[0][0]; //NP modified C.K. begin
  ...
  else if ((strcmp(word,"tqz") == 0) && atom->torque_flag) newtree->array = &atom->torque[0][2]; //NP modified C.K. end
}
...
int VarReader::read_scalar(char *str)
{
  ...
  if (me == 0) {
    while (1) {
      ...
      if ((ptr = strchr(str,'#'))) *ptr = '\0';          // strip comment //NP modified R.B.
