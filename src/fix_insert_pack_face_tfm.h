...
FixStyle(insert/pack/face/tfm,FixInsertPackFaceTFM)

#else

#ifndef LMP_FIX_INSERT_PACK_FACE_TFM_H
#define LMP_FIX_INSERT_PACK_FACE_TFM_H
...
class FixInsertPackFaceTFM : public FixInsert {
 public:

  FixInsertPackFaceTFM(class LAMMPS *, int, char **);
  ~FixInsertPackFaceTFM();
 ...
 protected:
  ...
  // massflow to be used
  class FixMassflowMeshFaceTFM *massflowface;
  char *idmassflowface;
  ...
  int counter_;
  double *fraction_face_local_all;
  double *min_face_extent_local_all;
  int nFaces_; // # number of magnification lens faces equal to the size of arrays from cfdemcoupling!
  int startIndex_; // # number of magnification lens faces equal to the size of arrays from cfdemcoupling!
  int endIndex_;
  ...
  bool warn_region;
  double cg_, cg3_, alphaMinSolid_;
  int type_offset;
 ...
 private:
  ...
  virtual void post_create_per_face_data_TFM();
